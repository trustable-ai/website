...
```bash
./generator.py            # regenerate content/aipps/, pulling the clones
...
The commit is confined to the three paths the build owns — `content/aipps/`,
`static/aipps/` and `docs/` — so unrelated edits in the working tree are never
...
- **icon** — copied to `static/aipps/<name>.png`. Two source layouts are in
...
`static/aipps/` after generation; otherwise they stay published forever, since
